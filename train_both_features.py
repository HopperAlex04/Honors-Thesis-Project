"""
Training script for model with both features enabled.

This script trains a model using both the "Highest Point Value in Hand" and
"Highest Point Value in Opponent Field" features. This is the full feature set
configuration.
"""
...
# Create environment with both features enabled
...
STATE_FILE = models_dir / "training_state_both_features.json"

# Checkpoint prefix to avoid conflicts with other training scripts
CHECKPOINT_PREFIX = "both_features_checkpoint"
...
print(f"\n{'='*60}")
print(f"TRAINING: Both Features Enabled")
print(f"Features: Hand=ON, Opponent Field=ON")
...
for x in range(start_round, rounds):
    ...
    try:
        Training.selfPlayTraining(trainee, trainee, eps_per_round, model_id=f"both_features_round_{x}_selfplay")
    except Exception as e:
    ...
    try:
        p1w, p2w = Training.selfPlayTraining(trainee, valid_agent, eps_per_round, True, model_id=f"both_features_round_{x}_vs_previous")
    except Exception as e:
    ...
    try:
        p1w, p2w = Training.selfPlayTraining(trainee, validation00, eps_per_round, True, model_id=f"both_features_round_{x}_vs_randomized")
    ...
    try:
        p1w, p2w = Training.selfPlayTraining(trainee, validation01, eps_per_round, True, model_id=f"both_features_round_{x}_vs_heuristic")
    ...
    try:
        p1w, p2w = Training.selfPlayTraining(trainee, validation02, eps_per_round, True, model_id=f"both_features_round_{x}_vs_gapmaximizer")
