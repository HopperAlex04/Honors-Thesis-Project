...
# Derived stats: trainee win rate per validation round (x = round, y = avg of trainee_first / trainee_second final win rates)
TRANEE_WIN_RATE_VS_GAPMAXIMIZER_KEY = "trainee_win_rate_vs_gapmaximizer"
TRANEE_WIN_RATE_VS_RANDOMIZED_KEY = "trainee_win_rate_vs_randomized"
...
def _is_randomized_path(path: Path) -> bool:
    """True if path stem indicates vs Randomized metrics (e.g. round_N_vs_randomized_trainee_first)."""
    return "randomized" in path.stem.lower()
...
def _series_trainee_wr_by_round(
    by_file: dict[Path, tuple[int, float]], max_points: int
) -> tuple[list[float], list[float]]:
    """One y per round: average win rates from trainee_first / trainee_second files for that round."""
    if not by_file:
        return ([], [])
    by_round: dict[int, list[float]] = defaultdict(list)
    for _path, (r, w) in by_file.items():
        by_round[r].append(w)
    x_list: list[float] = []
    y_list: list[float] = []
...
class MetricsReader:
    ...
    def __init__(self, stat_keys: list[str], max_points: int = DEFAULT_MAX_POINTS):
        ...
        # Derived: trainee win rate per round vs opponent; one wr per file (trainee_first / trainee_second), averaged per round
        self._trainee_wr_by_file: dict[Path, tuple[int, float]] = {}  # GapMaximizer: path -> (round_num, wr)
        self._trainee_wr_randomized_by_file: dict[Path, tuple[int, float]] = {}  # Randomized
    ...
    def poll(self) -> None:
        ...
        for path in self._paths:
            ...
            try:
                # Per-round tracking for trainee win rate vs validation opponent (one point per round file)
                round_wins: int | None = None
                round_games: int | None = None
                track_wr_vs_opponent = (
                    (TRANEE_WIN_RATE_VS_GAPMAXIMIZER_KEY in self.stat_keys and _is_gapmaximizer_path(path))
                    or (TRANEE_WIN_RATE_VS_RANDOMIZED_KEY in self.stat_keys and _is_randomized_path(path))
                )
                round_num = _round_sort_key(path)[0] if track_wr_vs_opponent else 0
                ...
                with open(path, encoding="utf-8") as f:
                    ...
                    for line in f:
                        ...
                        # Derived stat: trainee win rate vs GapMaximizer or Randomized per round (one point per round file)
                        if track_wr_vs_opponent:
                        ...
                        # NOTE: For "training-only" stats we want x-axis = training episode index.
                        # However, we must increment that index once per *episode record* (JSONL line),
                        # not once per stat key. Otherwise the x-axis is multiplied by the number
                        # of training-only keys present in the same record (e.g. DQN logs `loss` +
                        # multiple `dqn_*` debug stats).
                        training_x_val: float | None = None
                        training_record_has_any_training_stat: bool = False
                        for key in self.stat_keys:
                            ...
                            if is_training_only_stat(key):
                                if validating:
                                    continue
                                if training_x_val is None:
                                    training_x_val = float(self._training_episode_index)
                                x_val = training_x_val
                                training_record_has_any_training_stat = True
                            else:
                                x_val = ep_global
                        ...
                        # Increment training index at most once per episode record.
                        if training_record_has_any_training_stat:
                    ...
                    if (
                        TRANEE_WIN_RATE_VS_RANDOMIZED_KEY in self.stat_keys
                        and _is_randomized_path(path)
                        and round_games is not None
                        and round_games > 0
                    ):
                        wr = round_wins / round_games
                        self._trainee_wr_randomized_by_file[path] = (round_num, wr)
        ...
        # Build trainee win rate series per opponent: average both seats per round, then sort by round
        if TRANEE_WIN_RATE_VS_GAPMAXIMIZER_KEY in self.stat_keys:
            xs, ys = _series_trainee_wr_by_round(self._trainee_wr_by_file, self.max_points)
            self._series[TRANEE_WIN_RATE_VS_GAPMAXIMIZER_KEY] = (xs, ys)
        if TRANEE_WIN_RATE_VS_RANDOMIZED_KEY in self.stat_keys:
            xs, ys = _series_trainee_wr_by_round(
                self._trainee_wr_randomized_by_file, self.max_points
            )
            self._series[TRANEE_WIN_RATE_VS_RANDOMIZED_KEY] = (xs, ys)
