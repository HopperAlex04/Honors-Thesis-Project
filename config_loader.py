...
def _parse_config(data: Any, path: Path) -> DashboardConfig:
    ...
    for i, item in enumerate(stats_raw):
        ...
        if y_label is not None and not isinstance(y_label, str):
            raise ConfigError(f"stats[{i}].y_label must be a string; got {type(y_label).__name__}")

        stats.append(StatEntry(key=key, label=label, graph_type=graph_type, y_label=y_label))
